var oInitialize = {
	...
	readSellerDetials : function(){
		$.getJSON('SellersData.json', function(data){
			$("#personalData").append('<li><img class = "profileImage" src ="' + data.profileImage + '"/></li>');
			$("#personalData").append('<li><h7 class = "box">'+data.name+'<br>' + data.status+'</h7></li>');
			$("#personalData").append('<li><img class = "autograph" src = "Images/Autograph.png"></li>');
			$("#personalData").append('<li><button class = "orderNowButton">Order Now(25$)</button></li>');
			...
			$("#headLine").append('<h4>'+data.headLine + '</h4>');
			$(".description p").append(data.description);
			$(".middlePicture").attr("src", data.mainImage);
		});
	},
